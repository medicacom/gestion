...
import { getQuestion, getReponse,getScore,verifReponse } from "../../../Redux/userDocumentReduce";
import {
  getVideo,
  getVideoDocument,
  fetchVideo,
...
function Visualisation({obj}) {
  ...
  const confirmScore = useCallback(async (sc,scFinal) => {
    setAlert(
      ...
      >
        <ul>
          <li> Score question : {sc} </li>
          <li> Score examen : {scFinal} </li>
        </ul>
        {/* Vous éte sure de supprime cette User? */}
      </SweetAlert>
    );
  ...
  useEffect(() => {
    ...
    verifFeuille();
    getScoreUser();
  }, [dispatch, mergeDocument, verifFeuille, getVideoU, getVideoVerif, id,getQuestionnaire,getScoreUser,verifToken]); 
  const submitQuestion = useCallback(
    async (data,quest,type) => {  
        dispatch(saveVideoUser({ idDoc: id, idUser: idUser }));
        var sc = await dispatch(verifReponse({data:data,id:id,idUser:idUser,count:quest.length,type:type}));
  ...
  function Document() {
    if(score <75){
      ...
      if(mergedPdfUrl !=="" && testExamen === false){
        return (
          <div>
            {scoreExamen <= 0?
              <Button
            ...
            <Button
              ...
              type="button"
              variant="success"
              onClick={()=>{confirmScore(score,scoreExamen)}}
            >
              Score
              {/* <span className="btn-label">
                <i className="fas fa-arrow-right"></i>confirmMessage(sc.payload.score,type)  
            ...
            {verif===true?
              <a href={mergedPdfUrl} download className="btn btn-info float-right"><i className="fas fa-download"></i>Télécharger</a>
            :""}
          </div>
        )
      }
